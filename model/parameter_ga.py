...
class ParameterGA(object):
	...
	def create(self):
		...
		self.toolbox.register("individual", tools.initCycle, creator.Individual, 
							  (self.toolbox.attr_layers, self.toolbox.attr_layer_size, self.toolbox.attr_epochs), n=isize)
	...
	def cost(self, ind):
		...
		base = MyLSTM(self.trainx.shape[1], vlayers, [vlayersize for _ in range(vlayers)], self.trainy.shape[1], epochs=vepochs, batch_size=100, fit_verbose=0)
		...
		error = MyLSTM(e_trainx.shape[1], elayers, [elayersize for _ in range(elayers)], e_trainy.shape[1], epochs=eepochs, batch_size=100, fit_verbose=0)
